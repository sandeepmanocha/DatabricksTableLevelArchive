...
from src.recovery import delete_archived_slice
...
dbutils.widgets.text("reason", "", "Reason (required when dry_run is false)")
...
if not dry_run and not reason:
    raise ValueError(
        "Widget 'reason' is required and must be non-whitespace when dry_run is false.",
    )

reason_arg = reason if reason else None

# COMMAND ----------
_log = logging.getLogger("caresource_archive.notebook.delete_archived_slice")
...
try:
    ...
    displayHTML(
        "<h3 style='color:#b00020'>Delete archived slice failed</h3>"
...
displayHTML(
    "<h3>Delete archived slice summary</h3>"
    "<table border='1' cellpadding='6' cellspacing='0' style='border-collapse:collapse'>"
    f"{rows_html}"
    "</table>"
)
