...
| F3.9 | Two-tier merge at runtime: global settings defaults → table-level overrides. Table configs can omit `retention_years` to inherit from `global_settings.default_retention_years` |
...
| F6.15 | **Incremental append mode** `[PENDING CUSTOMER DISCUSSION]` — when year folder exists and `delete_after_archive=true`, append newly-eligible records to the existing year Delta table using Spark write mode `append`. Source only has records that were excluded in previous runs; no deduplication needed. Audit logs with `record_count` reflecting only the appended records. Metadata written with `"mode": "append"` |
