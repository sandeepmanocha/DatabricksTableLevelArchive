# 12 — Orphan Folder Detection

**Goal:** Archive detects when a folder exists but has no matching audit entry, logs FAILED with `orphan_folder` reason, then recovers after the orphan is removed.
...
**Expect:**
- Year 2018: STARTED → FAILED with error describing the orphan condition ("Archive folder exists at ... but no successful archive is recorded in the audit table")
- Error message includes resolution steps (inspect folder, delete if corrupt, re-run)
...
> **Note:** The internal reason code is `orphan_folder` (set on the `ArchiveOperationError.reason` attribute in `archiver.py`), but the audit log's `error_message` column stores the human-readable diagnostic text, not the reason code.

---

## Phase 3 — Fix + Re-run (recovery)

### 3a. Delete the orphan folder
